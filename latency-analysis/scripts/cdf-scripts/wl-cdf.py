...
for i in range(len(all_folders)):
        ...
        mu = frame2[' WriteLatency1'].mean()

        sigma = frame2[' WriteLatency1'].std()
        ...
        n_bins = np.arange(max(frame2[' WriteLatency1']) +1)

        n, bins, patches = ax.hist(frame2[' WriteLatency1'], n_bins, density=True, histtype='step', cumulative=True, label='Empirical')
        ...
        ax.set_xlabel('Write Latency (microseconds)')

        ax.set_ylabel('Likelihood of occurrence')

        ax.set_yticks(np.arange(0,1.01,step=0.1))

        plt.savefig("{}_WriteLatency-CDF.png".format(i))
