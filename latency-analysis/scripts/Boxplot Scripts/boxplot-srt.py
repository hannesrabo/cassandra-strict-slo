...
for i in range(len(all_folders)):
        if (all_folders[i].split('_')[2]!="hightarget"):
                if int(all_folders[i].split('_')[3]) in all_folders_box:
                        all_folders_box[int(all_folders[i].split('_')[3])].append(all_folders[i])
                else:
                        all_folders_box[int(all_folders[i].split('_')[3])]=[]
                        all_folders_box[int(all_folders[i].split('_')[3])].append(all_folders[i])
...
for i in range(len(all_folders_box)):
    ...
    print("\n")
    print("==================== SRT: "+str(folders_box_keys[i])+" ================================")
    print("\n")
...
plt.xlabel("Speculative Retry Threshold")
plt.ylabel("Read Latency")
plt.title("Latency per Speculative Retry Threshold")
plt.savefig("Boxplot-rl-srt.png", bbox_inches='tight')
...
plt.title("Latency per Speculative Retry Threshold")
plt.savefig("Boxplot-wl-srt.png", bbox_inches='tight')
