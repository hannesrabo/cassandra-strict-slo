...
ycsb2_cmd_load = "python bin/ycsb load cassandra2-cql -p hosts='100.0.0.11,100.0.0.12,100.0.0.13,100.0.0.14' -P workloads/workloadb -threads 12 -s"
...
ycsb1_cmd_run = "python bin/ycsb run cassandra2-cql -p hosts='100.0.0.11,100.0.0.12,100.0.0.13,100.0.0.14' -P workloads/workloadb -threads 12 -s -p cassandra.speculative=" + threshold
print("\nRunning workloads")
client1_out = open(rfile+"_Client1",'a') 
client2_out = open(rfile+"_Client2",'a') 
ycsb1 = subprocess.Popen(shlex.split(ycsb1_cmd_run),stdout=client1_out,stderr=client1_out)
ycsb2 = subprocess.Popen(shlex.split(ycsb2_cmd_run),stdout=client2_out,stderr=client2_out) 
...
while ycsb1.poll()==None:
    ...
    time.sleep(60)
    p.resume()
    print("Resumed client2"+str(datetime.now()))
    if ycsb1.poll()!=None:
       break
    time.sleep(60)
#time.sleep(6000)
print('YCSB1 has finished Running Waiting for YCSB2 to finish')
